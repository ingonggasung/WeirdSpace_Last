...
public class Kit_Tele_Yes : MonoBehaviour
{
    int stageIndex;
    ...
    IEnumerator FadeAndTeleport(Collider2D other)
    {
        ...
        {
            int randomIndex1 = Random.Range(2, 27);
            stageIndex = randomIndex1;
        }
        ...
        {
            SceneManager.LoadScene("kitchen Clear"); // 적절한 씬 이름 입력
            gameManeger.GameCount = 8;
        }
        else if (gameManeger.GameCount < 7) // 8회 이하일 때
        {
            SceneManager.LoadScene("Kit_" + stageIndex);
